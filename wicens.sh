...
# FW check
if [ "$build_no" != '386' ] || [ "$build_no" = '384' ] && [ "$build_sub" -lt 15 ] ; then
	F_terminal_header
	F_terminal_check_fail "Sorry this version of firmware is not compatible, please update to 384.15 or newer to utilize this script"
	F_terminal_padding
	exit 0
fi
...
if [ ! "alias | grep -q 'wicens='" ] ; then
	(alias "wicens='sh /jffs/scripts/wicens.sh'") &
fi
...
F_opt_backup_restore() {
	F_backup() {
		if [ -f /jffs/addons/wicensconfig.bak ] ; then
			while true; do
				...
				case "$configremove" in
					y|Y) rm -f /jffs/addons/wicensconfig.bak ; printf "%b" "$tBACK$tERASE" ;;
		...
		touch /jffs/addons/wicensconfig.bak
		echo "wicens backup created $(date +%c) - ver:$script_version - DO NOT EDIT THIS FILE" >> /jffs/addons/wicensconfig.bak
		if {   # start file output
			...
			} >> /jffs/addons/wicensconfig.bak ; then
				F_terminal_check_ok "Backup successful, saved to /jffs/addons/wicensconfig.bak"
		else
				F_terminal_check_fail "Backup failed, could not output to /jffs/addons/wicensconfig.bak"
		fi
	}
	...
	F_restore() {
		...
		if [ ! -f /jffs/addons/wicensconfig.bak ] ; then
			F_terminal_check_fail "Error, no backup found in /jffs/addons/wicensconfig.bak"
			F_menu_exit

		else
			F_config_line() { head -n "$1" /jffs/addons/wicensconfig.bak | tail -n1 ;}   # pass a line number to write
		...
		F_terminal_show "Remember to save /jffs/addons/wicensconfig.bak somewhere safe"
	}
	...
	F_uninstall_do() {
		...
		if [ -f "/jffs/addons/wicensconfig.bak" ] ; then 
			...
			case "$backupremove" in
				r|R) rm -f /jffs/addons/wicensconfig.bak && F_terminal_check_ok	"Removed /jffs/addons/wicensconfig.bak" ;;
				*) F_terminal_check_ok "Keeping /jffs/addons/wicensconfig.bak" ;;
			esac
		fi
...
F_build_settings() {
	...
	case "$setupwait" in
		t|T) rm -f /tmp/wicens.lock && exec sh /jffs/scripts/wicens.sh test ;;
		*) ;;
	esac
...
F_disable_autorun() {
	...
	if cru l | grep -q "wicens.sh cron" ; then
		cru d "wicens"
	...
	if [ -f "/jffs/scripts/services-start" ]; then
		if grep -q "wicens.sh cron" "/jffs/scripts/services-start" 2> /dev/null; then
	...
	if [ -f "/jffs/scripts/wan-event" ]; then
		if grep -q "wicens.sh wancall" "/jffs/scripts/wan-event" 2> /dev/null; then
			sed -i '/wicens.sh wancall/d' "/jffs/scripts/wan-event"
...
F_auto_run_check() {
	...
	F_serv_start_check() {
		if [ -f "/jffs/scripts/services-start" ]; then
			...
			if grep -q "$cron_current_setting" "/jffs/scripts/services-start" ; then
				...
				if [ "$user_cron_interval" = 'minute' ]; then
					echo "cru a wicens \"*/$user_cron_period * * * * /jffs/scripts/wicens.sh cron\"   # added by wicens" >> /jffs/scripts/services-start
				elif [ "$user_cron_interval" = "hour" ]; then
					echo "cru a wicens \"* */$user_cron_period * * * /jffs/scripts/wicens.sh cron\"   # added by wicens" >> /jffs/scripts/services-start
				fi
			...
			if [ "$user_cron_interval" = 'minute' ]; then
				echo "cru a wicens \"*/$user_cron_interval * * * * /jffs/scripts/wicens.sh cron\"   # added by wicens" >> /jffs/scripts/services-start
			elif [ "$user_cron_interval" = 'hour' ]; then
				echo "cru a wicens \"* */$user_cron_interval * * * /jffs/scripts/wicens.sh cron\"   # added by wicens" >> /jffs/scripts/services-start
			fi
	...
	F_wan_event_check() {
		if [ -f "/jffs/scripts/wan-event" ]; then
			...
			F_terminal_check "Checking for wicens entry"
			if grep -q '/jffs/scripts/wicens.sh wancall' "/jffs/scripts/wan-event" ; then
				printf "\r%b %s %bexists%b\n" "$tERASE$tCHECKOK" "$(grep '/jffs/scripts/wicens.sh wancall' "/jffs/scripts/wan-event" | cut -c -62)" "$tGRN" "$tCLR"
				...
				echo "[ \"\$2\" = 'connected' ] && sh /jffs/scripts/wicens.sh wancall &   # added by wicens" >> /jffs/scripts/wan-event
	...
	if [ "$user_cron_interval" = 'hour' ]; then
		cron_current_setting="\* \*/$user_cron_period \* \* \* /jffs/scripts/wicens.sh cron"
...
F_update_script() {
	...
	if sed -i "1,/saved_wan_ip=.*/{s/saved_wan_ip=.*/saved_wan_ip='$current_wan_ip'/;}" "$script_name_full"; then
		...
		if grep -q "$current_wan_ip" /jffs/scripts/wicens.sh; then
...
F_main_menu() {
	...
	grep -q 'wicens.sh cron' 2> /dev/null '/jffs/scripts/services-start' && printf "%bActive%b\n" "$tGRN" "$tCLR" || printf "%bDisabled%b\n" "$tRED" "$tCLR"
	printf "%b wan-event connected status---:       " "$tTERMHASH"
	grep -q 'wicens.sh wancall' 2> /dev/null '/jffs/scripts/wan-event' && printf "%bActive%b\n" "$tGRN" "$tCLR" || printf "%bDisabled%b\n" "$tRED" "$tCLR" 
...
if [ -f "/tmp/wicens.lock" ]; then
	...
	F_terminal_header
	F_terminal_show "wicens.sh failed to start"
	F_terminal_padding
	...
	if [ -f "/tmp/wicenssendmail.lock" ]; then   # if wicens.lock doesnt exist neither should this, so only check this if first lock exists
		...
		if [ "$(($(date +%s) - $(sed -n '4p' /tmp/wicenssendmail.lock)))" -gt "$check_lock_count" ]; then
			...
			if [ ! -d "/proc/$locked_process" ]; then # process that created doesnt exist
				...
				F_terminal_show "Use 'sh /jffs/scripts/wicens.sh remove'"
